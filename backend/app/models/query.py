...
class ClientQuery(Base):
    ...
    business_segment_id = Column(Integer, ForeignKey("business_segments.id"), nullable=True)
    ...
    responses = relationship("QueryResponse", back_populates="query", order_by="QueryResponse.created_at")
...
class QueryResponse(Base):
    ...
    is_internal_note = Column(Integer, default=0)  # 0=client-facing, 1=internal
    created_at = Column(DateTime(timezone=True), server_default=func.now())
